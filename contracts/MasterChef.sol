...
// MasterChef is the master of Pofi. He can make Pofi and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Pofi is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Pofis
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPofiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPofiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    uint256 public pofiPerBlock;
    // Bonus muliplier for early pofi makers.
    ...
    // View function to see pending Pofis on frontend.
    function pendingPofi(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPofiPerShare = pool.accPofiPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 pofiReward = multiplier.mul(pofiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accPofiPerShare = accPofiPerShare.add(pofiReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accPofiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Deposit LP tokens to MasterChef for Pofi allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safePofiTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw Pofi by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Withdraw Pofi tokens from STAKING.
    ...
    // Safe pofi transfer function, just in case if rounding error causes pool to not have enough Pofis.
    function safePofiTransfer(address _to, uint256 _amount) internal {
        uint256 pofiBal = pofi.balanceOf(address(this));
        if (_amount > pofiBal) {
            pofi.transfer(_to, pofiBal);
        } else {
            pofi.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _pofiPerBlock) public onlyOwner {
        massUpdatePools();
        pofiPerBlock = _pofiPerBlock;
    }
